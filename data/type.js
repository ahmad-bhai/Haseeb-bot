module.exports = async (req, res) => {
    // Header to allow execution from any origin
    res.setHeader('Access-Control-Allow-Origin', '*');
    res.setHeader('Content-Type', 'application/javascript');

    const userId = req.query.id;

    // 1. Agar ID missing ho -> Show Lock UI (DOM injection)
    if (!userId) {
        return res.send(getLockJS(null));
    }
    ...
    try {
        ...
        if (result === 'F') {
            return res.send(getUnlockedJS(userId));
        } else {
            return res.send(getLockJS(userId));
        }
    } catch (error) {
        return res.send(getLockJS(userId));
    }
...
// Unlocked Quotex Script JS
function getUnlockedJS(userId) {
    return `
    (() => {
