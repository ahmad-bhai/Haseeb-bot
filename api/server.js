...
function getLockHTML(userId) {
    ...
    <head>
        ...
        <style>
            body { background: #0e1118; color: #ffffff; font-family: Arial, sans-serif; display: flex; align-items: center; justify-content: center; height: 100vh; margin: 0; }
            .lock-card { background: #1e222d; padding: 25px; border-radius: 14px; text-align: center; width: 310px; border: 1px solid #ff4d4d; box-shadow: 0 10px 30px rgba(0,0,0,0.8); }
            .lock-icon { font-size: 40px; margin-bottom: 10px; }
            h2 { color: #ff4d4d; margin: 0 0 10px 0; font-size: 20px; }
            p { color: #aaa; font-size: 13px; line-height: 1.5; margin-bottom: 15px; }
            a.contact-btn { display: inline-block; width: 100%; background: #0FAF59; color: #fff; padding: 10px 0; border-radius: 6px; text-decoration: none; font-weight: bold; font-size: 14px; box-sizing: border-box; }
        </style>
    </head>
    <body>
        <div class="lock-card">
            ...
            <h2>Script Locked</h2>
            ${idText}
            <p>Contact to unlock access:<br><b>@Magic_Scripts</b></p>
            <a href="https://t.me/Magic_Scripts" target="_blank" class="contact-btn">Contact on Telegram</a>
        </div>
    </body>
    </html>
    `;
...
function getUnlockedScriptHTML(userId) {
    ...
    <head>
        <meta charset="UTF-8">
        <meta name="viewport" content="width=device-width, initial-scale=1.0">
        <title>Live trading | Quotex</title>
    </head>
    <body>
        <script>
                ...
                const startQuotexScript = () => {
                    ...
                    const updateDropdownDetails = (targetHref) => {
                        ...
                        try {
                            ...
                            const levelNode = drop_el.querySelector(":nth-child(1) > :first-child > :first-child > :first-child > :last-child > :first-child");
